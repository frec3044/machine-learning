...
We will tune two of them in our model (`trees` and `min_n`).  Setting the hyper-parameter equal to `tune()` is a placeholder that says we plan to tune it.  We also set `num.threads = parallel::detectCores()` so that the tuning uses the power of your computer to the fullest by assigns different model runs to different computer cores (laptops often have 4-8 cores).
