...
This extends the example in `tutorial1-machine-learning-101.qmd` to include hyperparameter tuning.
...
This step is the same as `tutorial1-machine-learning-101.qmd`
...
To tune the model's hyperparameters, we will divide the training data into "folds."
...
We will use the random forest model to predict biomass. The function [`rand_forest`](https://parsnip.tidymodels.org/reference/rand_forest.html) defines the model.  
...
This model has 3 tuning hyperparameters:
...
- `min_n`: Minimal Node Size (type: integer, default: see below). `min_n` depends on the mode. For regression, 5 is the default. For classification, a value of 10 is used.

We will tune two of them in our model (`trees` and `min_n`).  Setting the hyperparameter to `tune()` is a placeholder indicating we plan to tune it.  We also set `num.threads = parallel::detectCores()` so that the tuning uses the power of your computer to the fullest by assigns different model runs to different computer cores (laptops often have 4-8 cores).
...
An important rule when training a model is never to use the training data to evaluate it.  This is because ML models can be so good that they "learn" the data.  Therefore, the error on the training data will, by "definition," be low.  Evaluation is always done on data that was not used in the model training.  We have two ways that we are doing this

1) We have divided the data into a training and testing set, with 20% of the data in the testing set.  The model will never be fit on the testing set; it will only be used to predict it.

2) We have divided the training set into 10 folds, where each fold has an analysis and assessment set.  Each fold is randomly different from the others, with different rows of the training set assigned to the analysis and assessment sets.  We can then train the model on the analysis set in each fold separately and evaluate using the assessment set.  We can then average the model performance from the 10 different assessments to get an idea of model performance even before we look at the testing data. We will use the folds to tune the model hyperparameters.
...
### Estimate best hyperparameters using tuning

The `tune_grid()` function will train the model and recipe defined in the workflow using a set of parameters on a subset of the training data (the analysis set).  It then calculates a metric that describes how well the model with that set of hyper-parameters predicts the "assessment" set.  

Setting the grid to 25 creates 25 combinations of the two hyperparameters (5 x 5) we are tuning.  The grid is developed using a sensible range of hyperparameters.  
...
The table below is sorted so that the best hyperparameter set (lowest RMSE) is at the top. You can see the values for `trees` and `min_n` that were used in the training and the mean RMSE (`mean`) across all 10 "folds" (`n` is the number of folds).  
...
Our workflow (model + recipe) needs to know which hyperparameters to use when fitting the model.  The `finalize_workflow` function updates the workflow to include the best hyperparameters.
...
We use the same approach as before to train the model on the **full** training data (i.e., it does not use 10-fold cross-validation)).
...
```{r}
biomass_full_fit <- final_workflow |> 
...
```{r}
new_predictions <- predict(biomass_fit, 
                           new_data = new_data)
```
