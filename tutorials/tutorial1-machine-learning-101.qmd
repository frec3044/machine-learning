...
### Step 0: Define question

We has address many questions in this class related to atmospheric, terrestrial, and aquatic ecosystems.

## Step 1: Obtain data

Read data into memory.  In the course, we have accessed data from CSV files, Excel files, databases, web addresses, and APIs. 
...
We only want to evaluate our model on data not used in its training. Otherwise, we can only say that the model is good at fitting the data that was used to train it. This is not helpful for understanding how it would perform as a tool to generate new predictions.
...
A workflow combines the model and recipe into a single "object" that can be used for training, testing, and predicting new data.
  ...
  - Define model type: linear regression, regression tree, neural net, etc.
...
## Step 4: Train model
  ...
  - Tune hyperparameters: hyperparameters are configuration settings that govern how a particular ML method is fit to data. They are called "hyper" because "regular" parameters are the parameters within the model learned by the ML method.  For example, a method called "random forecast" requires a hyper-parameter that controls the minimum size of a regression tree that is allowed.  This parameter (called `min_n`) can be provided directly by you or tuned.  The tuning process involves repeatedly fitting the model with different hyperparameter values and selecting the values that yield the best fit to the data. Importantly, not all ML methods have hyperparameters (e.g., linear regression using the `lm` engine does not have hyperparameters).  We don't tune hyperparameters in this example. See `tutorial2-example-with-tuning.qmd` for an extension of this application that includes hyperparameter tuning.
  
  - Fit model (using best hyper-parameter if they are tuned).  The model is fit to the training data.

## Step 5: Predict

  - Predict testing data using the model that was fit to the training data. This step is critical because we want to evaluate our model only on data it has not seen before.
  - Predictions are quite easy using the `predict()` function.
  
## Step 6: Evaluate model

  - It is important to use the appropriate metrics to evaluate how the model performs.  Some metrics apply only to classification problems, and others apply only to regression problems. A list of metric types can be found [here](https://yardstick.tidymodels.org/articles/metric-types.html)
  We will focus on root-mean-squared error (`rmse`), a metric that subtracts each observation from the predictions, squares the differences, averages them across all data points, and then takes the square root.  
...
In the example application, we will use the vegetation carbon stock data that was used in the carbon stock model. Our goal is to predict the mean carbon stock of each plot in the NEON data. This will allow us to build a model that can predict carbon stocks at locations beyond NEON and potentially serve as a predictive tool for land managers.
...
The data are from the National Ecological Observatory Network, the DayMet project (meteorology data), and NASA's MODIS satellite.  They have been combined to provide woody-vegetation carbon stocks for each NEON plot, along with associated meteorology, land-use classification, and the normalized vegetation index (from MODIS).

```{r}
biomass_data <- read_csv("../assignment/data/neon_biomass.csv", show_col_types = FALSE)
```
...
We are going to split the data into training and testing sets using the `initial_split` function. `prop = 0.80` indicates that 80% of the data will be used for training``. Since we want the training and test sets to represent all `nlcdClass` types``, we include it as a `strata`.
...
**Important note: you will not always use a strata (this has confused students in the past).** If your data does not have groupings (e.g., categorical variables) that you want to have evenly distributed in the training and testing set, you do not need to use a strata.  This case you would have `split <- initial_split(biomass_data, prop = 0.80)`)
...
We need to create the model and engine that we will use. In this example, we are using `linear_reg` in `regression` mode (as opposed to `classification` mode). Setting the mode to linear regression is not necessary, as it only allows linear regression, but it is included here for completeness. 

The engine is `lm` because we are using the standard R function `lm`.  We could use other functions for linear regression modeling, and they would be specified as a different engine.
...
Now we will predict the test data using the model fitted to the training data.
...
Use the full data set to train your model.

```{r}
biomass_full_fit <- biomass_wflow |> 
  fit(data = biomass_data)
```
...
```{r}
new_data <- read_csv("../assignment/data/neon_biomass_new.csv", show_col_types = FALSE)
...
As in "Step 5: Predict Test Data", use the model fitted on the training data (`biomass_fit`) to predict the new data.

```{r}
new_predictions <- predict(biomass_full_fit, new_data = new_data)
```

Now create a data frame with the new data, predictions, model type, and your name. 
...
Your assignment is `assignment/assignment.qmd`, which challenges you to develop your own machine learning models to predict the carbon stock data from the forest carbon module.  You will be selected the best of the models that you developed and submit your predictions to the class prediction challenge.
