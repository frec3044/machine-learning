...
- Your two models can't be both `linear_reg` or `rand_forest` (we have already used these).  You can use one or the other as one of your two models, just not both. 

- You must use more predictors than used in the `tutorial/tutorial1-machine-learning-101.qmd` and `tutorial/tutorial2-example-with-tuning.Rmd`.  You do not have to use all predictors.  In fact, one of the decisions that you will be making is which predictors you will include.
...
**Question 11:** What two model and engines did you select to use?
...
**Question 12:** Provide a high level description of the modeling approachs you choose in Question 11 (max of two sentences each)
...
**Question 13:** Do either models require any special data pre-processing? If so, describe the steps needed.
...
**Question 14:** List any hyper-parameters that can be tuned in the models
...
**Question 17:** Provide code that defines the recipe for feature engineering. 
...
**Question 18:** Provide code that defines your two models (model type + engine).
...
**Question 20:** Provide code that tunes the hyper-parameters of your model (if your model has hyper-parameters you are required tune at least one of them).  If your model+engine does not have hyper-parameters that can be tuned, then state that as your answer.
...
Use your *best model* to predict new data. The new data is a data frame with all the same columns as the training and testing set above but with `NA` values for the biomass. I have the true biomass values and will calculate `rmse` for your predictions. Therefore you need to predict the biomass and upload your CSV to Canvas and I will download them as a set and run the analysis in class to examine the "scores"
