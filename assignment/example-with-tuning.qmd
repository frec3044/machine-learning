...
We will tune two of them in our model (`mtry` and `min_n`).  Setting the hyperparameter equal to `tune()` is a placeholder that says we plan to tune it. We also set `num.threads = parallel::detectCores()` so that the tuning uses your computer's power to the fullest.
