...
We will be using the metric root-mean-squared error to measure the quantify of the model fit (`metrics = metric_set(rmse)`)
...
In total the random forest model was run 25 * 10 times (25 different hyperparameter sets x 10 different folds of the training data).   This is why we used `num.threads = parallel::detectCores()` because assigns different model runs to different computer cores (laptops often 4-8 cores).

The table below that is sorted so that best hyperparameter set (lowest RMSE) is at the top.  You can see the values for `mtry` and `min_n` that were used in the training and mean RMSE (`mean`) across all 10 "folds" (`n` is the number of folds).  
